...
That entry exists because a session already tried to revert. LAEA – Lambert Azimuthal Equal Area – was the wrong call in one session, and a later one (possibly Claude Code trying to fix something unrelated) tried to bring it back. The entry heads off a third round of that – the wasted time, the burnt tokens – by naming the choice, the reason, and the wrong turn, so the next session just declines it.

::: {#fig-polaroid}
![](assets/ai-workflow/polaroid.webp){fig-alt="A hand holding a Polaroid photograph, its image black and undeveloped, a handwritten note reading CRS: NZTM – DON'T REVERT attached below." width="75%"}
...
None of these came from the system noticing its own failure. Each came from a person seeing the failure, working out the cause, and writing the fix back into the files. The system doesn't fix itself. A person spots the failure and works out the cause – then the file makes that fix stick, so the next session inherits it instead of repeating the failure.
