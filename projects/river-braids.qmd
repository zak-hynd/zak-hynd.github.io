...
![](assets/river-braids/change-map-crop.webp){fig-alt="..." width="100%"}

Pre- to post-flood channel change on the lower Waimakariri, active braid zone. Blue: newly wet. Orange: newly dry. Grey: unchanged. One flood, one map – where the braids moved and where they stayed.
:::
...
::: {#fig-state-maps .figure}
![](assets/river-braids/state-maps.webp){fig-alt="..." width="100%"}

From signal to state: SAR backscatter (top), true colour and NDWI (middle rows), and the aggregated per-pixel state map (bottom), pre- and post-flood. The state map is built from every usable pass in the window, not a single scene.
:::
...
::: {#fig-change-maps .figure}
![](assets/river-braids/change-map.webp){fig-alt="..." width="100%"}

Two reaches, two behaviours. Focus Area West (top): lateral braid migration across the wide braidplain. Focus Area East (bottom): channel widening and inundation in a narrower section.
:::
